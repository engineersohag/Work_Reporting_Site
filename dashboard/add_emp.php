...
if (isset($_POST['add_emp'])) {
  ...
  if(strlen($pass) < 4) {
    $_SESSION['error'] = "Password must have minimum 4 cher long";
    ...
    if($rows) {
      $_SESSION['error'] = "<small class='text-center bg-danger text-light p-2'>User ID already exist</small>";
      header("location:add_emp.php");
    }else{
      $sql = "INSERT INTO `user_tbl`(`fullname`, `user_des`, `user_scale`, `user_res`, `user_id`, `user_pass`, `usert_role`) VALUES ('$fullname','$des','$scale','$res','$user_id','$pass','$role')";

      $query = mysqli_query($con, $sql);

      if($query) {
        $_SESSION['success'] = "<small class='text-center bg-light text-success p-2'> Data Inserted Successfull</small>";
        header("location:add_emp.php");
      }
    }
  }
