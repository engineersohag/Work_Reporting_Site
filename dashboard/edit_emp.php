...
if (isset($_POST['update_emp'])){
  ...
  $user_des = mysqli_real_escape_string($con, $_POST['user_des']);
  $user_res = mysqli_real_escape_string($con, $_POST['user_res']);
  $user_scale = mysqli_real_escape_string($con, $_POST['user_scale']);
  $user_role = mysqli_real_escape_string($con, $_POST['user_role']);

  $update = "UPDATE `user_tbl` SET `fullname`='$fullname',`user_des`='$user_des',`user_scale`='$user_scale',`user_res`='$user_res',`usert_role`='$user_role' WHERE `id`='$id'";
  $query = mysqli_query($con, $update);

  if ($query){
    $_SESSION['update'] = "<p class='text-center text-success'>Data Updated Successfully</p>";
  ...
  <form action="" method="POST">
    <div class="row p-4 register_form border border-secondary">
      ...
      <div class="col-md-5 m-auto">
        ...
        <div class="mb-2">
          <label>User ID</label>
          <input readonly type="text" name="user_id" class="form-control" value="<?= $result['user_id'] ?>" maxlength="30" minlength="4"> </div>
        <div class="mb-2">
          <label>Password</label>
          <input disabled type="password" name="user_pass" class="form-control" maxlength="100" minlength="4"> </div>
        <div class="mb-2">
          <label>User Role</label>
          <select required name="user_role" class="form-control">

            <?php $role = $result['usert_role'] ?>
