...
if (isset($_POST['login_btn'])) {
  ...
  if($rows) {
    $recode = mysqli_fetch_assoc($query);
    $u_data = array($recode['fullname'], $recode['user_des'], $recode['user_scale'], $recode['user_res'], $recode['usert_role'], $recode['id']);

    $_SESSION['u_data'] = $u_data;

    $role = $recode['usert_role'];
    if($role == 0) {
